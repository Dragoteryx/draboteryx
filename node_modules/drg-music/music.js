...
exports.MusicHandler = function(cl) {
	...
	this.removeMusic = (guild, index) => {
		return new Promise((resolve, reject) => {
			...
			resolve(playlists.get(guild.id).playlist.remove(index).info());
		});
	}
	...
	this.resumeMusic = guild => {
		return new Promise((resolve, reject) => {
			...
			try {
				...
				reject("musicAlreadyPaused");
			}
			resolve(this.playingInfo(guild));
		});
	}
	...
	this.playlistInfo = guild => {
		...
		return playlists.get(guild.id).playlist.info();
	}
	this.playingInfo = guild => {
		if (!this.isPlaying(guild))
			throw new Error("notPlayingMusic");
		return playlists.get(guild.id).playlist.playingInfo();
	}
	...
	this.musicInfo = (guild, index) => {
		...
		return playlists.get(guild.id).playlist.musicInfo(index);
	}
