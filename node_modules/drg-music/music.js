...
exports.queryYoutubeInfo = async (query, ytbApiKey) => {
	...
	if (link instanceof Error) return Promise.reject(link);
...
exports.MusicHandler = function(client) {
	...
	this.addMusic = (request, member, options) => {
		return new Promise(async (resolve, reject) => {
			...
			// youtube query
				...
				reject(new Error("invalidInputType"));
		});
	}
	...
	this.removeMusic = (guild, index) => {
		return new Promise((resolve, reject) => {
			...
			if (this.isPlaylistEmpty(guild))
				reject(new Error("emptyPlaylist"));
			if (index < 0 || index >= playlists.get(guild.id).playlist.size())
				reject(new Error("invalidPlaylistIndex"));
			resolve(Object.freeze(playlists.get(guild.id).playlist.remove(index).info()));
		});
	}
	...
	this.nextMusic = guild => {
		return new Promise((resolve, reject) => {
			...
			if (!this.isPlaying(guild))
				reject(new Error("notPlayingMusic"));
			let current = this.playingInfo(guild);
			playlists.get(guild.id).playlist.skip();
			resolve(Object.freeze(current));
		});
	}
	...
	this.shufflePlaylist = guild => {
		return new Promise((resolve, reject) => {
			...
			if (this.isPlaylistEmpty(guild))
				reject(new Error("emptyPlaylist"));
			playlists.get(guild.id).playlist.shuffle();
			resolve();
		})
	}
	...
	this.clearPlaylist = guild => {
		return new Promise((resolve, reject) => {
			...
			if (this.isPlaylistEmpty(guild))
				reject(new Error("emptyPlaylist"));
			playlists.get(guild.id).playlist.clear();
			resolve();
		});
	}
	...
	this.toggleMusic = guild => {
		return new Promise((resolve, reject) => {
			...
			if (!this.isPlaying(guild))
				reject(new Error("notPlayingMusic"));
			resolve(playlists.get(guild.id).playlist.toggle());
		});
	}
	...
	this.pauseMusic = guild => {
		return new Promise((resolve, reject) => {
			...
			if (!this.isPlaying(guild))
				reject(new Error("notPlayingMusic"));
			try {
				playlists.get(guild.id).playlist.pause();
			} catch(err) {
				reject(err);
			}
			resolve(Object.freeze(this.playingInfo(guild)));
		});
	}
	...
	this.resumeMusic = guild => {
		return new Promise((resolve, reject) => {
			...
			if (!this.isPlaying(guild))
				reject(new Error("notPlayingMusic"));
			try {
				playlists.get(guild.id).playlist.resume();
			} catch(err) {
				reject(err);
			}
			resolve(Object.freeze(this.playingInfo(guild)));
	...
	this.setVolume = (guild, volume) => {
		return new Promise((resolve, reject) => {
			if (!this.isConnected(guild))
				reject(new Error("clientNotInAVoiceChannel"));
			if (volume < 0)
				reject(new Error("invalidVolume"));
			resolve(playlists.get(guild.id).playlist.setVolume(volume));
		});
	}
	...
	this.toggleLooping = guild => {
		return new Promise((resolve, reject) => {
			...
			if (!this.isPlaying(guild))
				reject(new Error("notPlayingMusic"));
			resolve(playlists.get(guild.id).playlist.toggleLooping());
		});
	}
	...
	this.togglePlaylistLooping = guild => {
		return new Promise((resolve, reject) => {
			...
			if (!this.isPlaying(guild))
				reject(new Error("notPlayingMusic"));
			resolve(playlists.get(guild.id).playlist.togglePlaylistLooping());
		});
	}
	...
	this.isPlaying = guild => {
		if (!this.isConnected(guild))
			return false;
		return playlists.get(guild.id).playlist.isPlaying();
	}
	this.isPaused = guild => {
		if (!this.isPlaying(guild))
			return false;
		return playlists.get(guild.id).playlist.isPaused();
	}
	this.isLooping = guild => {
		if (!this.isPlaying(guild))
			return false;
		return playlists.get(guild.id).playlist.loop;
	}
	this.isPlaylistLooping = guild => {
		if (!this.isPlaying(guild))
			return false;
		return playlists.get(guild.id).playlist.plLoop;
	}
	this.playlistInfo = guild => {
		if (!this.isPlaying(guild))
			throw new Error("notPlayingMusic");
		return Object.freeze(playlists.get(guild.id).playlist.info());
	}
	this.playingInfo = guild => {
		if (!this.isPlaying(guild))
			throw new Error("notPlayingMusic");
		return Object.freeze(playlists.get(guild.id).playlist.playingInfo());
	}
	this.playlistSize = guild => {
		if (!this.isConnected(guild))
			throw new Error("clientNotInAVoiceChannel");
		return playlists.get(guild.id).playlist.size();
	}
	this.isPlaylistEmpty = guild => {
		if (!this.isConnected(guild))
			return false;
		return playlists.get(guild.id).playlist.isEmpty();
	}
	this.musicInfo = (guild, index) => {
		if (playlists.get(guild.id).playlist.size() == 0)
			throw new Error("emptyPlaylist");
		if (index < 0 || index >= playlists.get(guild.id).playlist.size())
			throw new Error("invalidPlaylistIndex");
		return Object.freeze(playlists.get(guild.id).playlist.musicInfo(index));
	}
	this.remainingTime = guild => {
		if (!this.isConnected(guild))
			throw new Error("clientNotInAVoiceChannel");
		return playlists.get(guild.id).playlist.remainingTime();
	}
}
