...
function queryYoutube(query, apiKey, nb) {
	if (nb === undefined) nb = 1;
	return new Promise((resolve, reject) => {
		youtubeSearch(query, {key: apiKey, maxResults: nb, type: "video"}, (err, res) => {
			if (err) reject(err);
			else {
				let links = [];
		...
		return new Promise((resolve, reject) => {
		    ...
		    if (options.type == "link") {
					...
					if (options.apiKey === undefined) reject(new Error("parameter 'options.apiKey' is undefined"));
						...
						queryYoutube(request, options.apiKey, 1).then(link => {
							if (link.length) reject(new MusicError("no query results"));
							else {
								options.type = "link";
								resolve(this.add(link[0], member, options));
							}
						}).catch(reject);
					}
