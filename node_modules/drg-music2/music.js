...
function queryYoutube(query, nb) {
	if (query === undefined) query = "";
	if (nb === undefined) nb = 1;
	return new Promise((resolve, reject) => {
		if (youtube === null) reject(new Error("you must provide a valid Youtube API Key using 'setYoutubeApiKey' to use this function"))
		else {
			youtube.searchVideos(query, nb).then(async res => {
				let videos = [];
...
function youtubePlaylist(link) {
	...
	return new Promise(async (resolve, reject) => {
		if (youtube === null) reject(new Error("you must provide a valid Youtube API Key using 'setYoutubeApiKey' to use this function"))
			...
			resolve({title: playlist.title, videos: videos});
		}
	})
}
...
class MusicHandler extends EventEmitter {
	...
	add(request, member, options) {
		...
		return new Promise((resolve, reject) => {
			...
			else if (typeof request != "string") reject(new TypeError("'request' must be a String"));
		    ...
		    if (options.type == "link") {
					...
					if (options.apiKey === undefined) reject(new Error("parameter 'options.apiKey' is undefined"));
						...
						queryYoutube(request, 1).then(link => {
							if (link.length) reject(new MusicError("no query results"));
							else {
								options.type = "link";
								resolve(this.add(link[0].link, member, options));
							}
						}).catch(reject);
					}
