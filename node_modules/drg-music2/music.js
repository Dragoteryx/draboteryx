...
class MusicHandler extends EventEmitter {
	...
	join(member) {
		let that = prv(this);
		return new Promise((resolve, reject) => {
			if (member === undefined) reject(new Error("parameter 'member' is undefined"));
			else if (!(member instanceof discord.GuildMember)) reject(new TypeError("'member' must be a Discord GuildMember"));
			else if (this.isConnected(member.guild)) reject(new MusicError("the client already joined a voice channel in this guild"));
			else if (member.voiceChannel === undefined) reject(new MusicError("this member is not in a voice channel"));
			else if (!member.voiceChannel.joinable) reject(new MusicError("the client can't join this voice channel"));
			else if (!member.voiceChannel.speakable) reject(new MusicError("the client is not authorized to speak in this channel"));
			else if (member.voiceChannel.full) reject(new MusicError("this voice channel is full"));
			else {
